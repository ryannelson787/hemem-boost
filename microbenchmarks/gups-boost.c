...
struct gups_args {
  ...
  void* hot_field;                  // pointer to start of thread's hot region
  void* cold_field;                  // pointer to start of thread's cold region
  uint64_t size_of_hot;
  uint64_t size_of_cold;
...
static void *do_gups(void *arguments)
{
  ...
  uint64_t *hot_field = (uint64_t*)(args->hot_field);
  uint64_t *cold_field = (uint64_t*)(args->cold_field);
  uint64_t i;
